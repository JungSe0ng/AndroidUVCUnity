...
public class MjpegServer : MonoBehaviour
{
	...
	bool pluginDecodesJpeg = false; //You want the plugin to decode the jpeg (probably not, unless doing image processing on every frame).
	bool pluginProvidesJpeg = true; //You want the plugin to provide the jpeg (maybe only false if pluginDecodesJpeg = true)

	bool showFrame = false; //this app shows the frame
	...
	public void StartCamera()
	{
		...
		fps = int.Parse(line[2]);
		//actually start the camera.  The resolution and fps should work, because the camera publishes them
		int res = handler.plugin.Call<int>("Start", cameraName, width, height, fps, 2, bandwidth, pluginDecodesJpeg, pluginProvidesJpeg);
		...
		{ 
			...
			expSliderChanged(); //force max exposure
	...
	{
		int maxExposure = (int)(1.0f / fps * 10000);
		exposureSlider.maxValue = maxExposure/1000.0f;
	...
	private void Update()
	{
		if (opened && started)
		{
			if (showFrame)
			{
				...
				else if (pluginProvidesJpeg)
				{
					lock (jpegLock)
					{
						if (cameraJpeg != null)
						{
							cameraTexture.LoadImage(cameraJpeg);
						}
					}
				}
			}
			float currTime = Time.time;
			float delta = currTime - lastPrintTime;
			if (delta > 1)
			{
				
				cameraText.text = "" + capturedFrames / delta + "," + processedFrames / delta;
	...
	public void handleJNI()
	{
		if (opened && started)
		{
			...
			{
				Debug.Log(capturedFrames);
				capturedFrames += currFrameNumber - lastFrameNumber;
				lastFrameNumber = currFrameNumber; 
				processedFrames++;
				
				if (pluginProvidesJpeg)
				{
					//cameraText.text = "" + currFrameNumber;
